...
config = {
    ...
    'batch_size': 128,
    'seed': 0,
    'data_path': r'E:\Code\emnist-letters-train.csv'
}
...
class Conv2D(Layer):
    def __init__(self, in_channels, out_channels, kernel_size, stride=1, pad=0, reg=0.0):
    ...
    def backward(self, grad):
        ...
        if self.reg > 0:
            self.grad_W += self.reg * self.W.reshape(self.C_out, -1)
...
class SGD:
    ...
    def update(self, model):
        ...
        if hasattr(model, 'layers'):
            layers_to_update.extend(model.layers)
        if hasattr(model, 'fc'):
            layers_to_update.extend(model.fc.layers)
...
model = CNN()
optimizer = SGD(lr=config['lr'], reg=config['reg'])
